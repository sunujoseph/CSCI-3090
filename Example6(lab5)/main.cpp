...
float angle = 0.0; //angle gobal var
float num = 1.0;
float lx;
float lz;
...
void display() {
	...
	int normalLoc;
	int eyeLoc;
	int Lposition; // new uniform var from example.fs
	int Lcolour;
	...
	Lposition = glGetUniformLocation(program, "Lposition");
	Lcolour = glGetUniformLocation(program, "colour");

	glUniform4f(Lcolour, lcx, lcy, lcz, lca); //color animating 

	glUniform3f(Lposition, lx, cy, lz); //light animating 
...
int main(int argc, char **argv) {
	GLFWwindow *window;
	//change to use example6c.vs and example6c.fs instead a
	if(argc > 1) {
		vertexName = argv[1];
	} else {
		vertexName = "c";
